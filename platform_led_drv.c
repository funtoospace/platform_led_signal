...
static int platform_led_fasync(int fd, struct file *filp, int mode)
{
	struct platform_led_dev *dev = filp->private_data;
	printk("platform_led_fasync\n");
	return fasync_helper(fd, filp, mode, &dev->async_queue);
}

static int platform_led_release(struct inode *inode, struct file *filp)
{
	platform_led_fasync(-1, filp, 0);
	return 0;
}
...
*/
	...
	if (val == 1)
	{
		...
		*gpfdat &= ~((1<<4) | (1<<5) | (1<<6));
		...
		printk(KERN_INFO "dev->async_queue: %d\n", dev->async_queue);
		if (dev->async_queue) {
